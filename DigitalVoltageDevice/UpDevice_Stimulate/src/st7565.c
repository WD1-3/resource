...
void Lcd12864_Init()
{
	...
	LcdSt7565_WriteCmd(0xC0);  //Common direction 
...
uchar Lcd12864_Write16CnCHAR(uchar x, uchar y, uchar *cn)
{
	...
	{	
		...
		for (wordNum=0; wordNum<(sizeof(CN16CHAR)/sizeof(struct Cn16CharTypeDef)); wordNum++)
		{
		    //--查询要写的字在字库中的位置--//
...
//	*cn = "8";
	...
	{	
		...
		x1 = x/0x10;   //由于X坐标要两句命令，分高低4位，所以这里先取出高4位
		x2 = x%0x10;          //去低四位
		//--设置X坐标--//
		LcdSt7565_WriteCmd(0x10 + x1);   //高4位
		LcdSt7565_WriteCmd(0x04 + x2);	//低4位

		for (wordNum=0; wordNum< (sizeof(CN8CHAR)/sizeof(struct Cn8CharTypeDef)); wordNum++)
		{
		    //--查询要写的字在字库中的位置--//
...
{
	unsigned char i,s[5];
	i= sizeof(CN8CHAR)/ sizeof(struct Cn8CharTypeDef);
	s[0]=i%10 + '0';
	s[1]=i/10 + '0';
	s[2] ='\0';
	Lcd12864_Init();
	
	Lcd12864_Write16CnCHAR(40,0,"计率频");
	Lcd12864_Write8CnCHAR(0,2,s);
	
	Lcd12864_Write8CnCHAR(0,6,"5432109876543210");
	while(1);
}
